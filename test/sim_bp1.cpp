...
void initializeVector(std::vector<contents>& vect, unsigned long int size, int bit_count) {
    ...
    for (int i = 0; i < size; i++) {
        contents item;
        // item.initial_counter = counter_value;
        ...
        vect[vect.size() - 1].index_in_binary = vect[vect.size() - 1].index_in_binary.substr(32 - bit_count,bit_count);
    }
}
...
int main(int argc, char *argv[]) {
    ...
    } else if (strcmp(params.bp_name, "hybrid") == 0) 
    {
        ...
        initializeVector(vect, pow(2, params.M2), 2);
        initializeVector(vect2, pow(2, params.M1), 2);
        N_bit.resize(params.N, '0');
    } else {
